...
def cnn_concat(opt, n_blocks, n_filters, input_shape1, input_shape2):
    ...
    y = create_jitter_shimmer(opt, n_blocks, n_filters=[64,128,256], input_shape=input_shape2, tag='jit')
...
def predict_by_model(opt, dnn, val_names, score_file, layer_name):
    ...
    for i in range(len(val_names[0])):
        ...
        with open(opt.spec_path + val_names[0][i] + '.xls3', 'r') as f:
             dat = read_xls(f)
             sample12, i12 = get_vector(dat, opt.window_size, opt.spec_path + val_names[0][i], i11)
        ...
        with open(opt.spec_path + val_names[1][i] + '.xls3', 'r') as f:
             dat = read_xls(f)
             sample22, i22 = get_vector(dat, opt.window_size, opt.spec_path + val_names[1][i], i21)
